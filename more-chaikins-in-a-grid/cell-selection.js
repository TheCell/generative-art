...
function updateAlgrithmDefaults() {
    if (options.cellSelection === CellSelectionAlgorithm.EmergeFromACenter) {
...
function emergeFromACorner(nrOfXCells, nrOfYCells, previousCellX, previousCellY) {
    var randomGridCellX, randomGridCellY;
    
    if (previousCellX === undefined && previousCellY === undefined) {
        randomGridCellX = grid.length-1; // TODO Randomize but same as previous spline
        randomGridCellY = grid[0].length-1;
    } else {
        var diffX = random(100) > 50 ? 1 : -1;
        var diffY = random(100) > 50 ? 1 : -1;
